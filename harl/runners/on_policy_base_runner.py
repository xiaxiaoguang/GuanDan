...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        # self.league_pool = {agent_id: [] for agent_id in range(self.num_agents)}
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            for step in range(self.algo_args["train"]["episode_length"]):
                ...
                trajectory_buffer = [[] for _ in range(self.num_agents)]  # temp buffers for each agent
                
                while True:
                    curr_player = self.envs.envs[0].curr_player
                    ...
                    if np.all(dones, axis=1):
                        final_rewards = rewards  # {agent_id: reward}
                        for agent_id in range(self.num_agents):
                            for i in range(len(trajectory_buffer[agent_id])):
                                tr = list(trajectory_buffer[agent_id][i])
                                tr[2] = final_rewards  # Replace reward
                                self.logger.per_step(tuple(tr))  # for logging only
                                self._insert(tuple(tr), agent_id)
                        total_sum += rewards
                        break
    ...
    def collect_with_league(self, step, train_agent_id, opponent_actors):
        action_collector = []
        action_log_prob_collector = []
        rnn_state_collector = []

        for agent_id in range(self.num_agents):
            actor = self.actor[agent_id] if agent_id == train_agent_id else opponent_actors[agent_id]
            
            action, action_log_prob, rnn_state = actor.get_actions(
    ...
    @torch.no_grad()
    def collect(self, step, agent_id=None):
    ...
    def _insert(self, data, agent_id):
        """Insert data into buffer."""
        (
            obs,  # (n_threads, n_agents, obs_dim)
            share_obs,  # (n_threads, n_agents, share_obs_dim)
    ...
    def eval(self):
        ...
        while True:
            ...
            # for agent_id in range(self.num_agents):
            agent_id = self.envs.envs[0].curr_player
