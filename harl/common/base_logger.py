...
class BaseLogger:
    ...
    def __init__(self, args, algo_args, env_args, num_agents, writter, run_dir):
        ...
        self.trainloss_file = open(
            os.path.join(run_dir, "trainloss.txt"), "w", encoding="utf-8"
        )
        self.writter.add_text("Training Details", str(self.args))
        self.writter.add_text("Algorithm Details", str(self.algo_args))
    ...
    def log_train(self, actor_train_infos, critic_train_info):
            ...
            # Log actor training info per agent
            for agent_id, train_info in enumerate(actor_train_infos):
                for k, v in train_info.items():
                    tag = f"agent_{agent_id}/{k}"
                    # Convert tensor to float if needed
                    if isinstance(v, torch.Tensor):
                        v = v.item()
                    self.writter.add_scalars(tag, {tag: v}, self.total_num_steps)
